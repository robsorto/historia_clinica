...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		if (empty($idpersona)) {
			$rspta = $persona->insertar($apaterno, $amaterno, $nombre, $fecha_nacimiento, $sexo, $estado_civil, $alergia, $intervenciones_quirurgicas, $vacunas_completas, $tipo_documento, $num_documento, $direccion, $telefono, $email, $ocupacion, $persona_responsable, $profesion);
			echo $rspta ? "Paciente registrado" : "Paciente no se pudo registrar";
		} else {
			$rspta = $persona->editar($idpersona, $apaterno, $amaterno, $nombre, $fecha_nacimiento, $sexo, $estado_civil, $alergia, $intervenciones_quirurgicas, $vacunas_completas, $tipo_documento, $num_documento, $direccion, $telefono, $email, $ocupacion, $persona_responsable, $profesion);
			echo $rspta ? "Paciente actualizado" : "Paciente no se pudo actualizar";
		}
		break;

	case 'desactivar':
		$rspta = false; //$persona->desactivar($idpersona);
		echo $rspta ? "Paciente Desactivado" : "Paciente no se puede desactivar";
		break;

	case 'activar':
		$rspta = false; //$persona->activar($idpersona);
	...
	case 'buscar':
		...
		$rspta = $persona->buscar($idpersona);
		//Codificar el resultado utilizando json
		echo json_encode($rspta);
		break;
}
